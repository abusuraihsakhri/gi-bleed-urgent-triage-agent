"""
Command-Line Interface for GIBleed Sentinel: Rockall, Blatchford & Endoscopic Hemostasis Audit Agent.
"""
import argparse
import csv
import json
...
from .agents import GIBleedCoordinator

coordinator = GIBleedCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="gi-bleed-urgent-triage-agent", description="GIBleed Sentinel: Rockall, Blatchford & Endoscopic Hemostasis Audit Agent")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  GIBLEED SENTINEL: ROCKALL, BLATCHFORD & ENDOSCOPIC HEMOSTASIS AUDIT AGENT")
        print(f"  Domain: Gastroenterology | Standard: ACG / AGA Clinical Guidelines & Chicago v4.0")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting GIBleed Sentinel: Rockall, Blatchford & Endoscopic Hemostasis Audit Agent on http://{args.host}:{args.port}")
